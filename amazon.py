...
async def amazon(product_details):
    ...
    agent = Agent(
        ...
        - if particular product is not available on amazon, then return a message "Product not available on Amazon"
...
def get_amazon_output(input):
    return asyncio.run(amazon(input))
